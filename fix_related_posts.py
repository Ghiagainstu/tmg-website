...
# Title translations for related posts
TITLE_TRANSLATIONS = {
    'ja': {
...
def fix_related_posts(html_path, lang):
    """Fix related posts titles to use translated titles."""
    content = html_path.read_text(encoding='utf-8')
    translations = TITLE_TRANSLATIONS.get(lang, {})
    changed = False
    
    for slug, new_title in translations.items():
        # Find related post with this slug
        old_pattern = f'href="/{lang}/blog/{slug}/" class="related-card">'
        if old_pattern not in content:
            continue
        
        # Find the title after this href
        title_match = re.search(
            rf'href="/{lang}/blog/{re.escape(slug)}/" class="related-card">.*?related-card__title">([^<]+)',
            content,
            re.DOTALL
        )
        ...
        if title_match:
            ...
            # Replace title
            old_title_pattern = f'related-card__title">{en_title}'
            new_title_pattern = f'related-card__title">{new_title}'
...
for lang in ['ja', 'ko']:
    ...
    for html in lang_dir.glob('*.html'):
        if fix_related_posts(html, lang):
            fixed += 1
