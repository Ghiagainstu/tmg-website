...
# KO uses "더보기 →" instead of "続きを読む →"
pattern = re.compile(r'href="/ko/blog/([^"]*)/"[^>]*>.*?post-card__excerpt">[^<]*</p>\s*<span class="post-card__read-more">더보기 →</span>\s*</a>', re.DOTALL)
...
def fix_card(m):
    ...
    old = '<span class="post-card__read-more">더보기 →</span>'
    new = f'<span class="post-card__read-time">{read_time}</span>\n    <span class="post-card__arrow">더보기 →</span>'
