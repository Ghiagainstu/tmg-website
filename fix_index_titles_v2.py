...
# English -> Japanese title translations for batch articles
TITLE_TRANSLATIONS = {
...
# English -> Korean title translations
TITLE_TRANSLATIONS_KO = {
...
def fix_titles(index_path, lang, translations):
    """Fix English titles in index with translated titles."""
    content = index_path.read_text(encoding='utf-8')
    changed = False
    
    for slug, new_title in translations.items():
        # Find the title in the index
        # Pattern: href="/lang/blog/slug/" ... post-card__title">en_title
        old_pattern = f'href="/{lang}/blog/{slug}/"'
        
        if old_pattern not in content:
            continue
        
        # Find the title after this href
        # Use regex to find the title
        title_match = re.search(
            rf'href="/{lang}/blog/{re.escape(slug)}/"[^>]*>.*?post-card__title">([^<]+)',
            content,
            re.DOTALL
        )
        ...
        if title_match:
            ...
            # Replace title
            old_title_pattern = f'post-card__title">{en_title}'
            new_title_pattern = f'post-card__title">{new_title}'
...
# Fix JA index
ja_idx = WEBSITE / 'ja/blog/index.html'
if ja_idx.exists():
    if fix_titles(ja_idx, 'ja', TITLE_TRANSLATIONS):
        print("Fixed JA index titles")
    else:
        print("No changes needed for JA index")

# Fix KO index
ko_idx = WEBSITE / 'ko/blog/index.html'
if ko_idx.exists():
    if fix_titles(ko_idx, 'ko', TITLE_TRANSLATIONS_KO):
        print("Fixed KO index titles")
    else:
        print("No changes needed for KO index")
